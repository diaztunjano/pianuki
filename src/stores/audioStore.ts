...
interface SettingsSlice {
  settings: {
    ...
    sfxEnabled: boolean            // whether game sound effects are audible
    sfxVolume: number              // 0..1, independent of mic input
  }
...
> = (set) => ({
  ...
  setSfxVolume: (volume) =>
    set(
      (draft) => {
        draft.settings.sfxVolume = volume
      },
      false,
      ...
      })),
      {
        ...
        migrate: (persistedState, version) => {
          ...
          if (version < 3) {
            ...
            state.settings.sfxVolume = state.settings.sfxVolume ?? 1.0
          }
          return state
        },
      },
